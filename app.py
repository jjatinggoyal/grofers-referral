...
def admin():
    ...
    conn = psycopg2.connect(dbname=DB_NAME, user=DB_USER, password=DB_PASS, host=DB_HOST, port=DB_PORT)
    cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
    cur.execute("create table users (username character varying not null, password character varying not null, refer_status bigint not null, refer_code character varying not null, referred_by character varying, primary key(username))")
    cur.execute("insert into users (username, password, refer_status, refer_code) values ('jatin', 'goyal', 0, 'jatin')")
    conn.commit()
    cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
    cur.execute("select * from users")
    formdata = cur.fetchall()
